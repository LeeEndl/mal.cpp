...
namespace mal {
	...
	public:
		...
		anime(const nlohmann::json& j) noexcept {
			if (is_null<std::string>(j["type"]) == "RateLimitException") rate_limited = true;
			else {
				...
				jp_title = is_null<std::string>(j["title_japanese"]);
	...
	/* deletes the copy when leaving the lambda/scope */
	using smart_anime = std::unique_ptr<anime>;
	...
	*/
	template<typename T>
	void anime_get(const T& name, const int& results, std::function<void(smart_anime)> callback) noexcept {
			...
			for (int page = 1; page <= (results + 24) / 25; ++page) {
				...
				*j = nlohmann::json(nlohmann::json::parse(*all_data));
				...
				for (const auto& data : (*j)["data"])
					callback(std::make_unique<anime>(std::move(anime(data))));
			}
			});
	}
